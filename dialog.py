...
from qgis.PyQt.QtWidgets import (
    ...
    QMessageBox, QDialogButtonBox, QSizePolicy, QSpacerItem,
    QTabWidget, QWidget
)
from qgis.PyQt.QtCore import Qt, QVariant, QFileInfo
from qgis.PyQt.QtGui import QFont, QColor, QPixmap, QIcon

from qgis.core import (
    QgsVectorLayer, QgsFeature, QgsGeometry, QgsPointXY,
    QgsProject, QgsCoordinateReferenceSystem, QgsField, QgsFields,
    QgsWkbTypes, QgsMemoryProviderUtils, QgsRasterLayer,
    QgsVectorFileWriter, QgsCoordinateTransformContext, QgsCoordinateTransform
)
from qgis.gui import QgsProjectionSelectionWidget, QgsMapCanvas
...
class CsvImporterDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.import_btn = btn_box.addButton("Importa e Salva in GeoPackage...", QDialogButtonBox.AcceptRole)
        ...
        self.cancel_btn = btn_box.addButton("Annulla", QDialogButtonBox.RejectRole)
    ...
    def _build_info_tab(self):
        ...
        if os.path.exists(qr_path):
            ...
            luca_img.setPixmap(pixmap.scaled(150, 150, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
            self.luca_qr_missing = True
            luca_img.setText("QR Code Mancante")
        luca_img.setAlignment(Qt.AlignCenter)
        luca_layout.addWidget(luca_img)
        
        self.luca_info = QLabel()
        self.luca_info.setAlignment(Qt.AlignCenter)
        ...
        if os.path.exists(logo_path):
            ...
            sarino_img.setPixmap(pixmap.scaled(150, 150, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
            self.sarino_logo_missing = True
            sarino_img.setText("Logo Mancante")
        sarino_img.setAlignment(Qt.AlignCenter)
        sarino_layout.addWidget(sarino_img)
        
        self.sarino_info = QLabel()
        self.sarino_info.setAlignment(Qt.AlignCenter)
    ...
    def _update_texts(self):
        t = {
            'it': {
                'win_title': "GeoCSV Mapper — GeoPackage & Mappa (v1.8)",
                ...
                'txt_desc': (
                    "Questo plugin è uno strumento avanzato per l'importazione massiva di punti tramite file CSV in QGIS.<br><br>"
                    "Offre un potente parser per il riconoscimento automatico sia delle coordinate decimali standard sia dei formati "
                    "sessagesimali più complessi (DMS). Implementa un'anteprima rapida su cartografia OpenStreetMap, verifica "
                    "in tempo reale la codifica del testo e supporta l'esportazione automatizzata verso il formato moderno GeoPackage."
                ),
                'role_luca': "<b>Ruolo</b>: Ideatore e Consulente Tecnico<br><b>Email</b>: <a href='mailto:lucasti1988@gmail.com'>lucasti1988@gmail.com</a><br><b>Cell</b>: 3474565539<br><b>LinkedIn</b>: <a href='https://linkedin.com/in/luca-casti-326359357'>luca-casti</a>",
                'role_sarino': "<b>Ruolo</b>: Sviluppatore e Publisher<br><b>Email</b>: <a href='mailto:sino.grande@gmail.com'>sino.grande@gmail.com</a><br><b>Sito Web</b>: <a href='https://sinocloud.it'>sinocloud.it</a><br><b>GitHub</b>: <a href='https://github.com/sag1687'>sag1687</a>",
            ...
            'en': {
                'win_title': "GeoCSV Mapper — GeoPackage & Map (v1.8)",
                'tab_import': "Import",
                ...
                'txt_desc': (
                    "This plugin is an advanced tool for the bulk import of points via CSV files in QGIS.<br><br>"
                    "It offers a powerful parser for automatic recognition of both standard decimal coordinates and "
                    "complex sexagesimal formats (DMS). It features a quick preview on OpenStreetMap cartography, real-time "
                    "text encoding verification, and automated export support to the modern GeoPackage format."
                ),
                'role_luca': "<b>Role</b>: Creator & Technical Consultant<br><b>Email</b>: <a href='mailto:lucasti1988@gmail.com'>lucasti1988@gmail.com</a><br><b>Mobile</b>: 3474565539<br><b>LinkedIn</b>: <a href='https://linkedin.com/in/luca-casti-326359357'>luca-casti</a>",
                'role_sarino': "<b>Role</b>: Developer & Publisher<br><b>Email</b>: <a href='mailto:sino.grande@gmail.com'>sino.grande@gmail.com</a><br><b>Website</b>: <a href='https://sinocloud.it'>sinocloud.it</a><br><b>GitHub</b>: <a href='https://github.com/sag1687'>sag1687</a>",
        ...
        self.tabs.setTabText(1, c['tab_info'])
        ...
        elif "Operativo" in self.status_icon.text() or "Operational" in self.status_icon.text():
            self.status_icon.setText(c['lbl_ok'])
        elif "Codifica" in self.status_icon.text() or "Encoding" in self.status_icon.text():
    ...
    def _browse_csv(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, c, "", "File CSV (*.csv *.txt *.tsv);;Tutti i file (*)" if self.lang == 'it' else "CSV Files (*.csv *.txt *.tsv);;All files (*)"
        )
        if not path:
            return
    ...
    def _autoselect_xy(self):
        ...
        x_keywords = ["x", "lon", "longitude", "lng", "east", "est", "easting", "coord_x", "x_coord"]
        y_keywords = ["y", "lat", "latitude", "north", "nord", "northing", "coord_y", "y_coord"]
    ...
    def _update_preview_map(self):
        ...
        if count > 0:
            ...
            if self.map_canvas.mapSettings().destinationCrs() != crs:
                xform = QgsCoordinateTransformContext()
                transform = QgsCoordinateTransform(crs, self.map_canvas.mapSettings().destinationCrs(), QgsProject.instance())
                try:
    ...
    def _parse_coordinate(self, val):
        ...
        m_end = re.search(r'(?:^|[^a-z0-9])\s*(nord|est|sud|ovest|west|n|e|s|w|o)\s*$', val_lower)
        if m_end:
            d = m_end.group(1)
            ...
            m_start = re.search(r'^\s*(nord|est|sud|ovest|west|n|e|s|w|o)\s*(?:$|[^a-z0-9])', val_lower)
            if m_start:
                d = m_start.group(1)
                if d in ['sud', 'ovest', 'west', 's', 'w', 'o']:
                    dir_mult = -1
    ...
    def _on_import_clicked(self):
        ...
        try:
            with open(self.csv_path, newline="", encoding=enc) as f:
                ...
                try:
                    ...
                    dialect = csv.excel
                reader = csv.DictReader(f, dialect=dialect)
                fieldnames = [fn.strip() for fn in (reader.fieldnames or []) if fn.strip()]
                all_rows = [{k.strip(): v.strip() for k, v in row.items() if k} for row in reader]
        except Exception as e:
        ...
        if save_path:
            ...
            error, err_msg, new_filename, new_layer = QgsVectorFileWriter.writeAsVectorFormatV3(
                mem_layer, save_path, QgsProject.instance().transformContext(), options
            )
            
            if error == QgsVectorFileWriter.NoError:
                final_layer = QgsVectorLayer(f"{save_path}|layername={layer_name}", layer_name, "ogr")
                if final_layer.isValid():
                    QgsProject.instance().addMapLayer(final_layer)
                    QMessageBox.information(self, t['msg_succ'], f"{t['msg_succ_gpkg']} {final_layer.featureCount()}")
                    self.accept()
                    return
            else:
                QMessageBox.warning(self, t['msg_err'], t['msg_err_gpkg'].format(err_msg))

        QgsProject.instance().addMapLayer(mem_layer)
        QMessageBox.information(self, t['msg_succ'], f"{t['msg_succ_temp']} {mem_layer.featureCount()}")
        self.accept()
